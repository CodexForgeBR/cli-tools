...
apply_config() {
    ...
    # Tasks validation
    [[ -z "$OVERRIDE_TASKS_VAL_AI" && -n "$CONFIG_TASKS_VAL_AI" ]] && TASKS_VAL_AI="$CONFIG_TASKS_VAL_AI" || true
    [[ -z "$OVERRIDE_TASKS_VAL_MODEL" && -n "$CONFIG_TASKS_VAL_MODEL" ]] && TASKS_VAL_MODEL="$CONFIG_TASKS_VAL_MODEL" || true

    # Iteration limits
    [[ -z "$OVERRIDE_MAX_ITERATIONS" && -n "$CONFIG_MAX_ITERATIONS" ]] && MAX_ITERATIONS="$CONFIG_MAX_ITERATIONS" || true
    [[ -z "$OVERRIDE_MAX_INADMISSIBLE" && -n "$CONFIG_MAX_INADMISSIBLE" ]] && MAX_INADMISSIBLE="$CONFIG_MAX_INADMISSIBLE" || true
    [[ -n "$CONFIG_MAX_CLAUDE_RETRY" ]] && MAX_CLAUDE_RETRY="$CONFIG_MAX_CLAUDE_RETRY" || true
    [[ -n "$CONFIG_MAX_TURNS" ]] && MAX_TURNS="$CONFIG_MAX_TURNS" || true
    [[ -n "$CONFIG_INACTIVITY_TIMEOUT" ]] && INACTIVITY_TIMEOUT="$CONFIG_INACTIVITY_TIMEOUT" || true

    # File paths
    [[ -n "$CONFIG_TASKS_FILE" ]] && TASKS_FILE="$CONFIG_TASKS_FILE" || true
    [[ -n "$CONFIG_ORIGINAL_PLAN_FILE" ]] && ORIGINAL_PLAN_FILE="$CONFIG_ORIGINAL_PLAN_FILE" || true
    [[ -n "$CONFIG_LEARNINGS_FILE" ]] && LEARNINGS_FILE="$CONFIG_LEARNINGS_FILE" || true

    # Features
    [[ -n "$CONFIG_ENABLE_LEARNINGS" ]] && ENABLE_LEARNINGS="$CONFIG_ENABLE_LEARNINGS" || true
    [[ -n "$CONFIG_VERBOSE" ]] && VERBOSE="$CONFIG_VERBOSE" || true
    ...
    [[ -z "$OVERRIDE_NOTIFY_CHAT_ID" && -n "$CONFIG_NOTIFY_CHAT_ID" ]] && NOTIFY_CHAT_ID="$CONFIG_NOTIFY_CHAT_ID" || true
}
